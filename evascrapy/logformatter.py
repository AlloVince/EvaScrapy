...
CRAWLEDMSG = u"Crawled (%(status)s) [%(request_label)s]%(request_flags)s (referer: %(referer)s)%(response_flags)s"
...
class LogFormatter(object):
    ...
    def crawled(self, request, response, spider):
        ...
        request_label = request.meta.get('log_context') or request
        return {
            'level': logging.INFO,
            'msg': CRAWLEDMSG,
            'args': {
    ...
    def download_error(self, failure, request, spider, errmsg=None):
        """Format downloader failures for the Scrapy version in use."""
        if errmsg:
            return {
                'level': logging.ERROR,
                'msg': DOWNLOADERRORMSG_LONG,
                'args': {'request': request, 'errmsg': errmsg},
            }
        return {
            'level': logging.ERROR,
