...
- 具体网站 Spider 必须以本项目内的本地调试副本为唯一开发源；修改、调试和测试只能在本项目内进行，不得直接修改业务项目仓库
- 本项目内的 Spider 副本始终保持最新；日常副本由 `.gitignore` 隔离，不得提交
- 只有用户明确启动或发送 `.ai/workflow/end.md` 时，才将本项目内最新的业务 Spider 同步到已确认的业务项目；未明确启动 `end.md` 前禁止同步
- 同步后仍保留本项目内的最新 Spider，不删除或移动本地 Spider 文件
