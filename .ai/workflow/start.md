...
开始前必须扫描本项目同级目录中名称匹配 `*.crawler` 的候选项目，并与用户确认本次业务项目（即使只有一个候选也必须确认）。具体网站 Spider 可作为被 `.gitignore` 隔离的本地调试副本保留；用户发送 `.ai/workflow/end.md` 时，将本次 Spider 改动复制到已确认的业务项目，同时保留本地副本。站点经验只写入已确认的业务项目，EvaScrapy docs/memory 只保留通用框架知识。
