...
**依赖：** oss2、minio、kafka-python、aliyun-mns、elasticsearch（按使用）  
**雷区：** LocalFile 用 `from_crawler` 挂 crawler；OSS/S3/Kafka/MNS 的 `process_item` 签名带 spider。Kafka 发送失败会抛错阻断。ES basic_auth 从 URL userinfo 解析。
