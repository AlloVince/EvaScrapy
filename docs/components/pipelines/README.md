...
| `NatsPipeline` | `APP_MQ_NOTIFY_NATS` | NATS publish；在 Scrapy asyncio reactor 中异步懒连接并复用 client |
...
**雷区：** LocalFile 用 `from_crawler` 挂 crawler；OSS/S3/Kafka/MNS/NATS 的 `process_item` 签名带 spider。Kafka 发送失败会抛错阻断。NATS 的 `process_item` 是异步方法，必须在 Scrapy asyncio reactor 中执行；连接成功后复用 client。ES basic_auth 从 URL userinfo 解析。
